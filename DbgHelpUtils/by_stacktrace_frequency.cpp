...
{
    by_stacktrace_frequency::by_stacktrace_frequency(view_type const view, allocation_stack_trace_helper const& helper, process_heaps const& process, std::map<uint64_t, process_heap_entry> const& allocated_entries, std::map<uint64_t, process_heap_entry> const& free_entries)
    : interface_process_heaps_statistic_view_impl(view, helper, process, allocated_entries, free_entries)
    {
    }

    std::experimental::generator<process_heaps_statistic_bucket_view> by_stacktrace_frequency::buckets() const
    {
    ...
    by_stacktrace_frequency::bucket_entries& by_stacktrace_frequency::get_bucket(std::map<uint64_t, bucket_entries>& buckets, process_heap_entry const& entry)
    {
    ...
    double by_stacktrace_frequency::calculate_bucket_range_count_percent(bucket_entries const& bucket, size_t const max_entries)
    {
        return static_cast<double>(bucket.entries.size()) / static_cast<double>(max_entries) * 100.0;
    }

    double by_stacktrace_frequency::calculate_bucket_range_size_percent(std::map<uint64_t, bucket_entries> const& buckets, uint64_t const key)
    {
    ...
    uint64_t by_stacktrace_frequency::sum_entries(std::vector<process_heap_entry> const& entries)
    {
