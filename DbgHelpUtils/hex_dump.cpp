...
namespace dlg_help_utils::hex_dump
{
    ...
    void hex_dump(std::wostream& os, void const* data, size_t const length, size_t const indent,
                  bool const write_header, size_t const bytes_per_row, uint64_t const offset)
    {
        ...
        auto const bytes_per_row_hex_size_bytes = static_cast<streamsize>(std::max<>(
            hex_string_size_for(bytes_per_row), static_cast<size_t>(2)));
        ...
        for (size_t row = 0; row * bytes_per_row < length; ++row)
        {
            ...
            {
                os << to_hex(static_cast<unsigned int>(values[bytes_per_row * row + column]),
                             bytes_per_row_hex_size_bytes, L'0', false) << L' ';
            }
