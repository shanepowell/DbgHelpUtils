...
floating_decimal_128 float_to_fd128(const float f) {
  uint32_t bits = 0;
...
floating_decimal_128 double_to_fd128(const double d) {
  uint64_t bits = 0;
...
enum  // NOLINT(performance-enum-size)
{
    ...
    LONG_DOUBLE_EXPONENT_BITS = 15
};

floating_decimal_128 long_double_to_fd128(const long double d) {
  uint128_t bits = 0;
...
floating_decimal_128 generic_binary_to_decimal(const uint128_t bits, const uint32_t mantissaBits, const uint32_t exponentBits, const bool explicitLeadingBit)  // NOLINT(misc-use-internal-linkage)
{
#ifdef RYU_DEBUG
  printf("IN=");
  ...
  if (e2 >= 0) {
    ...
    const int32_t k = static_cast<int32_t>(FLOAT_128_POW5_INV_BITCOUNT + pow5bits(static_cast<int32_t>(q)) - 1);
    ...
    const int32_t k = static_cast<int32_t>(pow5bits(i) - FLOAT_128_POW5_BITCOUNT);
    ...
    } else if (q < 127) { // TODO(ulf jack): Use a tighter bound here.
  ...
  const int32_t exp = static_cast<int32_t>(e10 + removed);
...
static int copy_special_str(char * const result, const floating_decimal_128& fd) {  // NOLINT(misc-use-anonymous-namespace)
  if (fd.mantissa) {
    memcpy(result, "NaN", 3);  // NOLINT(bugprone-not-null-terminated-result)
    return 3;
  }
  if (fd.sign) {
    result[0] = '-';
  }
  memcpy(result + fd.sign, "Infinity", 8);  // NOLINT(bugprone-not-null-terminated-result)
  return fd.sign + 8;
}
...
int generic_to_chars(const floating_decimal_128& v, char* const result) {
  ...
  for (uint32_t i = 0; i < olength - 1; ++i) {
    const uint32_t c = static_cast<uint32_t>(output % 10);
    output /= 10;
  ...
  if (olength > 1) {
    result[index + 1] = '.';
    index += static_cast<int>(olength + 1);
  } else {
    ++index;
  }
  ...
  // Print the exponent.
  result[index++] = 'E';
  int32_t exp = static_cast<int32_t>(v.exponent + olength - 1);
  if (exp < 0) {
    result[index++] = '-';
    exp = -exp;
  }
