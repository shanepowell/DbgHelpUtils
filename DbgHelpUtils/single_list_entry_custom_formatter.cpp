...
{
    std::wstring const& single_list_entry_custom_formatter::symbol_name = common_symbol_names::single_list_entry_structure_symbol_name;
    const std::unordered_map<std::wstring_view, single_list_entry_custom_formatter::known_name_type> single_list_entry_custom_formatter::g_known_names =
        {
    ...
    single_list_entry_custom_formatter::single_list_entry_custom_formatter(cache_manager& cache)
    : cache_manager_{&cache}
    {
    }

    bool single_list_entry_custom_formatter::is_custom_type(
    ...
    {
        return is_type_equal(walker, type, path, name, parents, get_cache_data(walker).single_list_entry_symbol_type);
    }

    symbol_type_utils::dump_variable_symbol_data_result single_list_entry_custom_formatter::format(
        ...
        , symbol_type_utils::symbol_data_dumper const& dumper)
    {
        ...
        if (walker.find_memory_range(variable_address, get_cache_data(walker).single_list_entry_symbol_type.length().value_or(1),1)  == 0)
        {
            return
            {
        ...
        {
            single_list_entry_walker list_entry_walker{cache(), walker, variable_address, it->second.symbol_name, it->second.field_name};
        ...
        single_list_entry_walker list_entry_walker{cache(), walker, variable_address};
    ...
    generator<symbol_type_utils::dump_variable_symbol_data> single_list_entry_custom_formatter::generate_list_children_of_type(
    ...
    generator<symbol_type_utils::dump_variable_symbol_data> single_list_entry_custom_formatter::generate_list_children(single_list_entry_walker const& list_entry_walker, i_value_type_formatter const& formatter)
    {
        uint64_t index = 0;
        for (auto const entry_address : list_entry_walker.entries())
        {
    ...
    single_list_entry_custom_formatter::cache_data const& single_list_entry_custom_formatter::get_cache_data(stream_stack_dump::mini_dump_memory_walker const& walker) const
    {
        if (!cache_data_)
        {
    ...
    single_list_entry_custom_formatter::cache_data const& single_list_entry_custom_formatter::setup_globals(stream_stack_dump::mini_dump_memory_walker const& walker) const
    {
