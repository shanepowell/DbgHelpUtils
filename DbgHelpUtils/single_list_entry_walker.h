...
namespace dlg_help_utils::ntdll_utilities
{
    ...
    {
    public:
        single_list_entry_walker(cache_manager& cache, stream_stack_dump::mini_dump_memory_walker const& walker, uint64_t start_address, std::wstring const& entry_symbol_name, std::wstring const& entry_field_name, std::function<uint64_t (uint64_t, uint64_t)> address_decoder = {});
        single_list_entry_walker(cache_manager& cache, stream_stack_dump::mini_dump_memory_walker const& walker, uint64_t start_address);
    ...
    private:
        struct cache_data
        {
            ...
            stream_utils::symbol_type_and_base_type_field_offset next_field_data;
        };
