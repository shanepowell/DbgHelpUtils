...
namespace dlg_help_utils::exception_utils
{
    ...
    std::vector<std::wstring> exception_flags_to_list(uint32_t const exception_flags)  // NOLINT(misc-use-internal-linkage)
    {
        return generate_flags_strings(exception_flags, exception_flags_map(), flags_string_utils::mask_used_flags_t{true});
    }

    namespace resources
    {

        std::unordered_map<uint32_t, std::wstring> const& get_exception_code_descriptions()  // NOLINT(misc-use-internal-linkage)
        {
            winrt::Microsoft::Windows::ApplicationModel::Resources::ResourceManager const rm{};
