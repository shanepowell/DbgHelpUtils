...
namespace dlg_help_utils::thread_info_utils
{
    ...
    {
        return generate_flags_string(dump_flags, dump_flag_masks(), flags_string_utils::mask_used_flags_t{true});
    }

    std::vector<std::wstring> dump_flags_to_strings(uint32_t const dump_flags)  // NOLINT(misc-use-internal-linkage)
    {
        return generate_flags_strings(dump_flags, dump_flag_masks(), flags_string_utils::mask_used_flags_t{true});
    }
