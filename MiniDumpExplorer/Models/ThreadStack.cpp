...
namespace winrt::MiniDumpExplorer::implementation
{
    ...
    fire_and_forget ThreadStack::LoadThreadStack(dlg_help_utils::mini_dump const& mini_dump)
    {
        ...
        {
            logger::Log().LogMessage(log_level::error, std::format(L"LoadThreadStack failed for thread [{0}]: {1}\n", threadId, e.message()));
        }
        catch (std::runtime_error const& e)
        {
            logger::Log().LogMessage(log_level::error, std::format("LoadThreadStack failed for thread [{0}]: {1}\n", threadId, e.what()));
        }
        ...
        {
            logger::Log().LogMessage(log_level::error, std::format("LoadThreadStack failed for thread [{}]", threadId));
        }
    }
    ...
    fire_and_forget ThreadStack::LoadThreadExStack(dlg_help_utils::mini_dump const& mini_dump)
    {
        ...
        {
            logger::Log().LogMessage(log_level::error, std::format(L"LoadThreadExStack failed for thread [{0}]: {1}\n", threadId, e.message()));
        }
        catch (std::runtime_error const& e)
        {
            logger::Log().LogMessage(log_level::error, std::format("LoadThreadExStack failed for thread [{0}]: {1}\n", threadId, e.what()));
        }
        catch (std::exception const& e)
        {
            logger::Log().LogMessage(log_level::error, std::format("LoadThreadExStack failed for thread [{0}]: {1}\n", threadId, e.what()));
        ...
        {
            logger::Log().LogMessage(log_level::error, std::format("LoadThreadExStack failed for thread [{}]", threadId));
        }
    }
}
