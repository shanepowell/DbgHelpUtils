...
void process_user_mode_dump(std::wostream& log, mini_dump const& dump_file, std::unique_ptr<mini_dump> const& base_diff_dump, dump_file_options& options)
{
    ...
    auto const x86 = is_x86_target_t{system_info_utils::is_wow64_process(dump_file) || system_info_utils::is_x86_process(dump_file)};
    value_type_formatter formatter{x86};
    symbol_type_utils::symbol_data_dumper symbol_data_dumper{formatter};
    ntdll_utilities::register_custom_formatters(symbol_data_dumper, cache);
    ...
    {
        dump_mini_dump_all_stream_indexes(log, dump_file, options, symbol_engine, symbol_data_dumper);
    }
    else
    {
        ...
        for (auto const& type : options.dump_stream_types())
        {
            try
            {
                dump_mini_dump_stream_type(log, dump_file, type, options, symbol_engine, symbol_data_dumper);
            }
            catch (wide_runtime_error const& e)
            {
    ...
    {
        dump_mini_dump_heap(log, dump_file, cache, options, symbol_engine, formatter);
    }

    if(options.display_heap_entries())
    {
        dump_mini_dump_heap_entries(log, dump_file, cache, base_diff_dump, options, symbol_engine, formatter);
    }
    ...
    {
        dump_mini_dump_heap_statistics(log, dump_file, cache, base_diff_dump, options, symbol_engine, formatter);
    }
    ...
    for (auto const& symbol_name : options.symbol_names())
    {
        try
        {
            dump_mini_dump_symbol_name(log, dump_file, symbol_name, options, symbol_engine, symbol_data_dumper);
        }
        catch (wide_runtime_error const& e)
        {
    ...
    for (auto const& address_type : options.dump_address_types())
    {
        try
        {
            dump_mini_dump_address(log, dump_file, address_type, options, symbol_engine, symbol_data_dumper);
        }
        catch (wide_runtime_error const& e)
        {
