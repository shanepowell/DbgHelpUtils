    ...
    , i_value_type_formatter const& formatter)
{
    ...
    log << std::format(L"{1:<{0}} NT/Segment   Flags      {3:<{2}} {4:<{2}} {5:<{2}} {6:<{2}} {7:<{2}} {8:<8} {9:<4}\n"
        , formatter.pointer_format_width() + 3, L"Address"sv
    ...
    for(uint32_t heap_index = 0; heap_index < peb.number_of_heaps(); ++heap_index)
    {
        ...
        if(auto const segment_signature = peb.segment_signature(heap_index); segment_signature == heap::SegmentSignatureNtHeap)
        {
            if(auto const nt_heap = peb.nt_heap(heap_index); nt_heap.has_value())
            {
                ...
                detail::print_nt_heap_segments_list(log, formatter, nt_heap.value(), 2);
                detail::print_nt_heap_lfh_segments_list(log, formatter, nt_heap.value(), 2);
            }
        }
    ...
    for(uint32_t heap_index = 0; heap_index < peb.number_of_heaps(); ++heap_index)
    {
        if(auto const nt_heap = peb.nt_heap(heap_index); nt_heap.has_value())
        {
            detail::print_nt_heap(log, formatter, options, nt_heap.value(), peb);
        }
        ...
        {
            detail::print_segment_heap(log, formatter, get_process_marker(detail::is_process_heap_t{peb.process_heap() == segment_heap.value().segment_heap_address()}), segment_heap.value(), options, 0);
        }
    }

    for(auto const& heap : heap::dph_heap::dph_heaps(cache, peb))
    {
        detail::print_debug_page_heap(log, formatter, heap, options, 2);
    }

    log << L"\nVirtual Allocation Memory Ranges:\n";
    log << std::format(L" {1:<{0}} {2:<{0}} {3:<{0}}\n", formatter.pointer_format_width() + 2, L"Start"sv, L"End"sv, L"Size"sv);
...
void dump_mini_dump_heap_entries(
    std::wostream& log
    , mini_dump const& mini_dump
    , cache_manager& cache
    ...
    , i_value_type_formatter const& formatter)
{
    ...
    {
        detail::print_process_entry(log, entry, heaps.peb(), formatter, options, 2);
    }
