...
namespace
{
    ...
    std::wstring get_node_specific_data(heap::process_heap_graph_heap_entry const& node, std::streamsize const hex_length)
    {
    ...
    template <typename T>
    void print_node_line(std::wostream& log, T const& node, size_t indent, std::optional<uint64_t> parent_offset, std::optional<uint64_t> pointer, auto const cycle_end_detected, std::streamsize const hex_length, bool const to_reference, bool const already_logged_children)
    {
        log << std::format(L"{0:{1}}{2}{3}{4}{5}\n", ' ', indent, parent_offset.has_value() ? (to_reference ? L"->"sv : L"<-"sv) : get_type_name<T>(), get_node_attributes(node, parent_offset, pointer), get_node_specific_data(node, hex_length), cycle_end_detected ? L" - cycle end"sv : (already_logged_children ? L" - already logged children"sv : L""sv));
    }
    ...
    void print_display_node(std::wostream& log, display_node const& display_node, std::streamsize const hex_length)
    {
        if(display_node.node.has_value())
        {
            std::visit([&log, &display_node, hex_length](auto const& _) mutable { print_node_line(log, _, display_node.indent, display_node.parent_offset, display_node.pointer, display_node.cycle_end_detected, hex_length, display_node.to_reference, display_node.already_logged_children); }, display_node.node.value());
        }
        else
        {
    ...
    void expand_all_child_nodes(dump_file_options const& options, nodes_index_map const& nodes_index, std::set<uint64_t>& printed_nodes, std::vector<display_node>& display_nodes)
    {
        bool all_expanded;
        do
        {
            ...
            for (auto it = display_nodes.begin(); it != display_nodes.end(); )
            {
                if(!it->expanded && it->node.has_value())
                {
                    ...
                    const auto copy_parents = it->parents;
    ...
    void generate_display_node_to_print(dump_file_options const& options, heap::process_heap_graph_entry_type const& node, nodes_index_map const& nodes_index, size_t indent, std::set<uint64_t>& printed_nodes, std::vector<display_node>& display_nodes)
    {
...
void dump_mini_dump_heap_graph(std::wostream& log, mini_dump const& mini_dump, cache_manager& cache, dump_file_options const& options, dbg_help::symbol_engine& symbol_engine)
{
    ...
    std::vector<display_node> display_nodes;
    for(auto const& node : nodes)
    {
    ...
    {
        print_display_node(log, display_node, hex_length);
    }
}
