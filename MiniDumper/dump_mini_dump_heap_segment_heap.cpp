...
namespace detail
{
    namespace
    {
        void print_heap_large_entry_single_line(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::large_alloc_entry const& entry
            , size_t const indent)
        {
        ...
        void print_heap_large_entry_debug(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::large_alloc_entry const& entry
            , size_t const indent)
            ...
            , size_t const indent)
        {
            ...
            {
                print_heap_large_entry_debug(log, formatter, entry, indent);
            }
            else
            {
                print_heap_large_entry_single_line(log, formatter, entry, indent);
            }
        ...
        void print_lfh_entry_single_line(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_lfh_entry const& entry
            , size_t const indent)
        {
            using namespace size_units::base_16;
            ...
            , size_t const indent)
        {
            if(options.debug_heap_data())
            {
                print_lfh_entry_debug(log, formatter, entry, indent);
            }
            else
            {
                print_lfh_entry_single_line(log, formatter, entry, indent);
            }
        ...
        void print_lfh_subsegment(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_lfh_subsegment const& subsegment
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            {
                print_lfh_entry(log, formatter, entry, options, indent + 2);
            }
        }

        void print_lfh_affinity_slot(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_lfh_affinity_slot const& affinity_slot
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            {
                print_lfh_subsegment(log, formatter, subsegment, options, indent + 2);
            }
        }

        void print_lfh_bucket_single_line(std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_lfh_bucket const& lfh_bucket
            , size_t const indent)
        {
            using namespace size_units::base_16;
            ...
            , size_t const indent)
        {
            if(options.debug_heap_data())
            {
                print_lfh_bucket_debug(log, formatter, lfh_bucket, indent);
            }
            else
            {
                print_lfh_bucket_single_line(log, formatter, lfh_bucket, indent);
            }
            ...
            if(lfh_bucket.is_enabled())
            {
                ...
                {
                    print_lfh_affinity_slot(log, formatter, affinity_slot, options, indent + 2);
                }
            }
        }

        void print_lfh_context_heap(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , const heap::heap_lfh_context& lfh_context
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            {
                print_lfh_bucket(log, formatter, bucket, options, indent + 2);
            }
        }
        ...
        void print_vs_entry_single_line(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_vs_entry const& entry
            , size_t const indent)
        {
            using namespace size_units::base_16;
            ...
            , size_t const indent)
        {
            if(options.debug_heap_data())
            {
                print_vs_entry_debug(log, formatter, entry, indent);
            }
            else
            {
                print_vs_entry_single_line(log, formatter, entry, indent);
            }
        ...
        void print_vs_subsegment(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_vs_subsegment const& subsegment
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            for(auto const& entry : subsegment.entries())
            {
                print_vs_entry(log, formatter, entry, options, indent + 2);
            }
        }

        void print_vs_context_heap(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::heap_vs_context const& vs_context
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            {
                print_vs_subsegment(log, formatter, subsegment, options, indent + 2);
            }

            log << std::format(L"{0}  Free Entries:\n", indent_str);
            for(auto const& entry : vs_context.free_entries())
            {
                print_vs_entry(log, formatter, entry, options, indent + 4);
            }
        }

        void print_segment_heap_page_entry_single_line(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::page_range_descriptor const& entry
            , size_t const indent)
        {
            using namespace size_units::base_16;
        ...
        void print_segment_heap_page_entry_debug(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , heap::page_range_descriptor const& entry
            , size_t const indent)
            ...
            , size_t const indent)
        {
            if(options.debug_heap_data())
            {
                print_segment_heap_page_entry_debug(log, formatter, entry, indent);
            }
            else
            {
                print_segment_heap_page_entry_single_line(log, formatter, entry, indent);
            }
        ...
        void print_segment_heap_page(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , size_t const page_index
            , heap::heap_page_segment const& page
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            if(options.debug_heap_data())
            {
                ...
                {
                    print_segment_heap_page_entry(log, formatter, entry, options, indent + 2);
                }
            }
            else
            {
        ...
        void print_segment_context_heap(
            std::wostream& log
            , i_value_type_formatter const& formatter
            , size_t const segment_context_index
            , heap::heap_segment_context const& segment_context
            , dump_file_options const& options
            , size_t const indent)
        {
            std::wstring const indent_str(indent, L' ');
            ...
            {
                print_segment_heap_page(log, formatter, index, page, options, indent + 2);
                ++index;
            }
            ...
            for(auto const& entry : segment_context.free_page_ranges())
            {
                ...
                print_segment_heap_page_entry(log, formatter, entry, options, indent + 4);
            }
        }
    }
    ...
    void print_segment_heap(
        std::wostream& log
        , i_value_type_formatter const& formatter
        , std::wstring_view const& process_heap_marker
        , heap::segment_heap const& segment_heap
        , dump_file_options const& options
        , size_t const indent)
    {
        std::wstring const indent_str(indent, L' ');
        ...
        {
            print_segment_context_heap(log, formatter, index, segment_context, options, indent + 2);
            ++index;
        }

        print_vs_context_heap(log, formatter, segment_heap.vs_context(), options, indent + 2);
        print_lfh_context_heap(log, formatter, segment_heap.lfh_context(), options, indent + 2);

        for(auto const& entry : segment_heap.large_entries())
        {
            print_heap_large_entry(log, formatter, entry, options, indent + 2);
        }
    }
}
